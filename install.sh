...
echo -e "      ${DIM}  and runs pyVid2-qt; adds ~/bin to PATH in ~/.bashrc${RESET}"
echo
...
cp -r "$ASSETS_DIR/ir_keymap.conf" "$DATA_DIR"
...
if grep -qF 'PATH.*HOME/bin\|PATH.*~/bin' "$HOME/.bashrc" 2>/dev/null; then
